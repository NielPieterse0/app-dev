...
function Test-BlockedCommand {
  ...
  $rules = @(
    ...
    @{ Name = "recursive PowerShell alias delete"; Pattern = "(?i)(^|[\s;&|])(ri|del|erase)\b(?=.*\s-(Recurse|r)\b)"; Reason = "Recursive PowerShell alias deletion is blocked. Use targeted edits or a reviewed cleanup script." },
    ...
    @{ Name = "node recursive deletion"; Pattern = "(?i)(^|[\s;&|])node\s+(-e|--eval)\b(?=.*(rmSync|rmdirSync|unlinkSync))(?=.*recursive\s*:\s*true)"; Reason = "Node recursive deletion snippets are blocked. Use targeted edits or a reviewed cleanup script." },
    @{ Name = "python recursive deletion"; Pattern = "(?i)(^|[\s;&|])python(3)?\s+(-c|-m)\b(?=.*shutil\.rmtree)"; Reason = "Python recursive deletion snippets are blocked. Use targeted edits or a reviewed cleanup script." },
    @{ Name = "gh release create"; Pattern = "(?i)(^|[\s;&|])gh\s+release\s+create\b"; Reason = "GitHub release creation must be performed manually after release review." },
    ...
    @{ Name = "npm version"; Pattern = "(?i)(^|[\s;&|])npm\s+version\s+(major|minor|patch|premajor|preminor|prepatch|prerelease|\d)"; Reason = "Version bumps can trigger release flows. Ask for explicit release intent first." },
    ...
    @{ Name = "docker system prune all"; Pattern = "(?i)(^|[\s;&|])docker\s+system\s+prune\b(?=.*\s-a(\s|$))"; Reason = "Docker system prune -a can remove broad local state. Ask for explicit cleanup approval." },
    @{ Name = "supabase db mutation"; Pattern = "(?i)(^|[\s;&|])supabase\s+db\s+(push|remote|deploy)\b"; Reason = "Supabase database mutations require explicit project/environment approval." },

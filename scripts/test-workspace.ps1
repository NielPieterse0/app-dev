...
try {
  ...
  if ($plan -match "{{APP_NAME}}|{{TEMPLATE}}|{{DATE}}") {
    Write-Error "Generated PLAN.md still contains unresolved template placeholders."
  }
  if ($plan -notmatch [regex]::Escape($projectName)) {
    Write-Error "Generated PLAN.md does not contain the project name."
  }

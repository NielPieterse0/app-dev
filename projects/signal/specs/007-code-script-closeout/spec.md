...
Close out all open code and script findings from the 2026-07-09 review cycle: the Linux portability defect class that turned CI red on PR 2, CI job hygiene and diagnosability, secret-scanner gaps, release-gate design, template parity for Signal, parameter-semantics normalization, and residual hygiene items. `standards/scripting.md` 1.0.0 is adopted and registered in workspace validation by this slice; this spec executes its section 10 adoption sequence items 1-4 and 6.

## Requirements

- R1 Portability class (scripting standard 2.1-2.5): fix `scripts/common.ps1` (Get-ActiveSpecRelativePath returns forward-slash paths), `scripts/test-hooks.ps1` (path literals; invoke `pwsh`, never the Windows-only binary name), `scripts/test-workflow-enforcement.ps1` (dot-source and fixture path literals), `scripts/get-workflow-obligations.ps1` (direct git invocation, `2>$null`), `scripts/validate-workflow-receipts.ps1` (script path literal), `scripts/analyze-spec.ps1` and `scripts/check-spec-artifacts.ps1` (separator round-trips; consolidate the active-spec regex onto the common helper), `scripts/create-app.ps1` (generated AGENTS.md uses forward-slash `../../scripts/` paths), and Signal `package.json` `release:check` (invoke `pwsh`).
...
- R6 Secret scanner: add detection patterns for modern key shapes with the `sb_secret_` prefix family; replace the line-pinned allowlist entry with a path-plus-pattern suppression carrying an inline justification, or remove it if the referenced file does not exist.
...
- R8 Template parity (scripting standard B.9): verify `projects/signal/src/lib/env.ts` parity with the fixed template `env.ts` (lazy parse, backend-key guards) and reconcile or record divergence; align SettingsLayout active-state handling with B.2 (router-derived) in template and app; record any accepted divergences.

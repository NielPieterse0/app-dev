...
- Each app keeps its own `AGENTS.md`, `PLAN.md`, `specs/`, and `package.json` scoped to its directory. Gated checklists live inside the active spec folder (`specs/NNN-<slug>/checklist.md`), not at the app root.
...
app-dev/
  ...
  projects/
    <app>/
      AGENTS.md
      PLAN.md
      specs/NNN-<slug>/     # spec.md, tasks.md, workflow-receipts.md, checklist.md
...
|---|---|---|
| Workflow phases, Lean/Gated paths, convergence | `standards/spec-driven-workflow.md` | AGENTS.md, SKILL.md, PLANS.md, README |
...
A change that duplicates an owned fact into a second file is a review-blocking defect even if the copies currently agree.
...
- `projects/<app>/AGENTS.md` - app identity, active-spec pointer, app-specific constraints, done-when list. Generated by `create-app.ps1`; paths inside it are forward-slash and repo-relative.
- `PLANS.md` - when a plan is required, minimum plan content, lifecycle, completion rule.
...
- `.agents/commands/` - the executable workflow: `specify` (includes the gated clarify branch), `plan`, `tasks`, `implement`, `analyze`, `verify` (includes convergence closure), `release-readiness`. Each command states its working directory, exact script invocations, receipt obligations, and closure rule.
...
- Execution: `.agents/commands/`. Enforcement: `check-spec-artifacts.ps1`, `analyze-spec.ps1`, `validate-workflow-receipts.ps1 -ProjectPath . -RequireVerificationEvidence`, `verify-app.ps1`.
...
Changes to `.codex/`, `.agents/`, root `AGENTS.md`, `PLANS.md`, `standards/`, the manifest, templates, CI, or `scripts/` are governance changes: run the canonical governance check sequence before commit, and keep governance-layer changes in slices separate from app feature slices except where a defect discovered by app work is fixed at its template or harness source in the same slice, with the pairing recorded in the receipt.
